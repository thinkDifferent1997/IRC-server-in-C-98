...
public:
    ...
    char getModeChar() const { return 'k'; }
    ...
    bool isActive(IChannel* channel) const { return !channel->getKey().empty(); }
    std::string getParam(IChannel* channel) const { return channel->getKey(); }
