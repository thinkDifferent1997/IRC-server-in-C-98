...
public:
    ...
    char getModeChar() const { return 't'; }
    bool requiresParamToSet() const { return false; }
    bool requiresParamToUnset() const { return false; }
	bool validateParam(const std::string& param) const;
    bool isActive(IChannel* channel) const { return channel->isTopicRestricted(); }
    std::string getParam(IChannel* channel) const { (void)channel; return ""; }
};
